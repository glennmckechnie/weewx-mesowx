...
class SyncService(weewx.engine.StdService):
    ...
    def __init__(self, engine, config_dict):
        ...
        self.http_retry_interval = float(self.sync_config.get('archive_http_retry_interval', 60.0))
        # number of times to retry http requests before giving up for a while (default: 10)
        self.http_max_tries = int(self.sync_config.get('archive_http_max_tries', 1))
        # time to wait in seconds before retrying after a failure (default: 15 minutes)
        self.failure_retry_interval = float(self.sync_config.get('archive_failure_retry_interval', 900.0))
        ...
        self.batch_size = int(self.sync_config.get('archive_batch_size', 300))
        ...
        while True:
            ...
            if len(batch) > 0:
                ...
                if post_status == 200:
                    ...
                    # Add start/end datetime to log message
...
class SyncThread(threading.Thread):
    ...
    def __init__(self, queue, engine, config_dict, exit_event, http_pool, thread_name="SyncThread", **sync_params):
        ...
        self.http_retry_interval = 60.0
        # default number of times to retry http requests before giving up for a while
        self.http_max_tries = 1
        # time to wait in seconds before retrying after a failure (default: 15 minutes)
        self.failure_retry_interval = float(sync_params.get('archive_failure_retry_interval', 900.0))
